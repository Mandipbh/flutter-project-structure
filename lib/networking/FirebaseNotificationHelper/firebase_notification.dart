...
List<String> firebasePreventNotificationList = [];
class FirebaseMessagesHelper{
  ...
  static bool checkIsDuplicateNotification(String notificationId,String methodName){
    if(!firebasePreventNotificationList.contains(notificationId)){
      firebasePreventNotificationList.add(notificationId);
      print("IS DUPLICATE ARRIVE NOT $notificationId from $methodName");
      return true;///Is Not duplicate
    } else{
      print("IS DUPLICATE ARRIVE $notificationId from $methodName");
      return false;///Is Already arrived
    }
  }
  ...
  static handleNotification(RemoteMessage message, bool fireLocalNotification,{String methodName = '',String notificationId = ''}){
    ...
    if (Platform.isIOS == false && message.notification != null && fireLocalNotification) {
      print("message data is ${message.data}");
        sendLocalNotification("Abc", "Test notification", 123, methodName: methodName, notificationId : "1234");
    }
  }
